...
const openAddImageToPostBtn = document.querySelector(
  "#profilePostModal #add-post-image"
);
const openGifPostBtn = document.querySelector(
  "#profilePostModal #add-post-gif"
);
const openBookPostBtn = document.querySelector(
  "#profilePostModal #add-post-book"
);

/// ADD IMAGE TO POST PREVIEW
let postImageUrl = [];
const dropZone = document.querySelector("#dropzone-wrapper");
const inputElement = document.querySelector("#dropzone-wrapper input");
const postImageUrlInputs = document.querySelector(
  "#dropzone-wrapper .postImageURL"
);
const addPostImg = document.querySelector("#dropzone-wrapper img");
const addPhotoInfo = document.querySelector("#dropzone-wrapper .dropzone-desc");
const photoTrashCan = document.querySelector(
  "#row-post-image #delete-post-photo i"
);

// EVENT LISTENERS
document.addEventListener("DOMContentLoaded", () => {
  initializeEmojiPostSection();
  initializeCreatePostModal();
  initializeImageButtons();
  initializeGifButtons();
  initializeGifSection();
  initializeBookButtons();
  initializeClickableBook();
});
...
dropZone.addEventListener("drop", (e) => {
  ...
  let imageUrl = e.dataTransfer.items[0].getAsString(function (url) {
    ...
    displayDroppedImage(url);
    console.log(url);

    // Update the hidden input with the data URL
    postImageUrlInputs.value = url;

    // Add the data URL to the imageUrls array
    postImageUrl.push(url);
  });
});
photoTrashCan.addEventListener("click", deleteAddPostPhoto);

/// OPEN AND CLOSE CREATE POST MODAL
function initializeCreatePostModal() {
  const openBtn = document.getElementById("create-post");
  const closeBtn = document.getElementById("create-post-cancel-btn");
  const modal = document.getElementById("createProfilePostModal");
  const postModal = document.getElementById("profilePostModal");

  openBtn.addEventListener("click", () =>
    openCloseCreatePostModal(modal, postModal, true)
  );
  closeBtn.addEventListener("click", () =>
    openCloseCreatePostModal(modal, postModal, false)
  );
}

function openCloseCreatePostModal(modal, postModal, isOpen) {
...
/////ADD EMOJIS
function initializeEmojiPostSection() {
  const inputText = document.querySelector("#create-post-textarea");
  const emojiInput = document.querySelector("#emoji i");

  const picker = createPopup(
    {},
    {
      referenceElement: inputText,
...
function initializeImageButtons() {
  ...
  const closeImageBtn = document.querySelector(
    "#profilePostModal .row-post-image #image-cancel-btn i"
  );
  const imageSection = document.querySelector(
    "#profilePostModal #row-post-image"
  );

  addImageBtn.addEventListener("click", () =>
    openAddImageToPost(imageSection, addImageBtn)
  );
  closeImageBtn.addEventListener("click", () =>
    closeAddImageToPost(imageSection, addImageBtn)
  );
}

function openAddImageToPost(imageSection, addImageBtn) {
  addImageBtn.classList.add("post-btn-active");
  imageSection.classList.add("visible");
  disablePostButtonsImage();
}

function closeAddImageToPost(imageSection, addImageBtn) {
...
function disablePostButtonsImage() {
  openGifPostBtn.style.pointerEvents = "none";
  openBookPostBtn.style.pointerEvents = "none";
}

function resetPostImagePreview() {
  addPostImg.src = "";
  addPostImg.alt = "";
  postImageUrlInputs.value = "";
  inputElement.value = "";
  postImageUrl = [];
  addPostImg.style.display = "none";
  addPhotoInfo.style.display = "block";
  photoTrashCan.style.display = "none";
}
...
function displayDroppedImage(url) {
  addPostImg.style.display = "block";
  addPhotoInfo.style.display = "none";
  addPostImg.src = url;
  addPostImg.alt = url;
  photoTrashCan.style.display = "block";
}
...
function displayImage(file) {
  ...
  reader.onload = function (e) {
    ...
    addPostImg.src = src;
    addPostImg.alt = file.name;
    photoTrashCan.style.display = "block";
  };
  reader.readAsDataURL(file);
}

function deleteAddPostPhoto() {
  addPostImg.src = "";
  addPostImg.alt = "";
  postImageUrlInputs.value = "";
  inputElement.value = "";
  ...
  addPostImg.style.display = "none";
  addPhotoInfo.style.display = "block";
  photoTrashCan.style.display = "none"; // Hide the trash can when there's no image
}

/// === DISPLAY GIF SECTION ====////
function initializeGifButtons() {
  const addGifBtn = document.querySelector("#profilePostModal #add-post-gif");
  const gifBackBtn = document.querySelector("#gif-back-btn");
  const profilePostModal = document.querySelector("#profilePostModal");
  const gifSelection = document.querySelector("#gifSelection");
  const targetGifSection = document.getElementById("row-post-gif");
  ...
  gifBackBtn.addEventListener("click", () =>
    toggleGifSection(profilePostModal, gifSelection, gifResultsContainer, false)
  );
}

function toggleGifSection(
  modal,
  gifSelection,
  targetGifSection,
  gifResultsContainer,
  ...
  if (show) {
    ...
    debounce(getTrendingData(window.giphyApiKey, gifResultsContainer), 300);
  }
}

async function getTrendingData(apiKey, gifResultsContainer) {
  ...
  try {
    ...
    generateHTML(data.data, gifResultsContainer); // Array of trending GIFs
  } catch (error) {
...
function initializeGifSection() {
  ...
  gifSearchInput.addEventListener(
    "input",
    debounce(() => {
      searchGIFs(apiKey, gifSearchInput.value, gifResultsContainer);
    }, 300)
  );
}

async function searchGIFs(apiKey, gifSearchInput, gifResultsContainer) {
  const searchQuery = gifSearchInput;

  if (!searchQuery) {
    // If search input is empty, go back to trending
    await getTrendingData(apiKey, gifResultsContainer);
    return;
  }
  ...
  try {
    ...
    const data = await response.json();
    console.log(data.data);
    generateHTML(data.data, gifResultsContainer); // Array of searched GIFs
  } catch (error) {
    console.error("Error searching GIFs:", error);
  }
}

function generateHTML(results, container) {
  const postModal = document.getElementById("profilePostModal");
  const gifSelection = document.querySelector("#gifSelection");
  ...
  document.querySelectorAll(".animated-gif").forEach((gifElement) => {
    gifElement.addEventListener("click", () => {
      moveGifToSection(gifElement);
      postModal.style.display = "block";
      gifSelection.style.display = "none";
      postModal.classList.add("active");
      gifSelection.classList.remove("active");
      openAddImageToPostBtn.style.pointerEvents = "none";
      openBookPostBtn.style.pointerEvents = "none";
    });
  });
}

function moveGifToSection(gifElement) {
  ...
  const deleteButton = createGifDeleteButton(clonedGifElement);

  // Create a hidden input element
  const hiddenInput = document.createElement("input");
  hiddenInput.type = "hidden"; // Set the input type to "hidden"
  hiddenInput.name = "postGIF"; // Set a name for the input element (change as needed)
  ...
  targetGifSection.appendChild(container);
}

function createGifDeleteButton(clonedGifElement) {
...
function initializeBookButtons() {
  const addBookBtn = document.querySelector("#profilePostModal #add-post-book");
  const bookBackBtn = document.querySelector("#book-back-btn");
  const profilePostModal = document.querySelector("#profilePostModal");
  const bookSelection = document.querySelector("#bookSelection");

  addBookBtn.addEventListener("click", () =>
    toggleBookSection(profilePostModal, bookSelection, true)
  );
  bookBackBtn.addEventListener("click", () =>
    toggleBookSection(profilePostModal, bookSelection, false)
  );
}

function toggleBookSection(modal, bookSelection, show) {
  modal.style.display = show ? "none" : "block";
...
function initializeClickableBook() {
  const postModal = document.getElementById("profilePostModal");
  const bookSelection = document.querySelector("#bookSelection");
  ...
  document
    ...
    .forEach((bookContainer) => {
      bookContainer.addEventListener("click", () => {
        ...
        postModal.classList.add("active");
        bookSelection.classList.remove("active");
        openAddImageToPostBtn.style.pointerEvents = "none";
        openGifPostBtn.style.pointerEvents = "none";
      });
    });
...
function moveBookToSection(
  ...
  const targetBookSection = document.getElementById("row-post-book");
  targetBookSection.innerHTML = "";
  ...
  const deleteButton = createBookDeleteButton(clonedBookElement);
  ...
  targetBookSection.appendChild(container);
}

function createBookDeleteButton(clonedBookElement) {
  const deleteButton = document.createElement("a");
  deleteButton.innerHTML = '<i class="fa-solid fa-xmark"></i>';
...
function enablePostButtons() {
  openGifPostBtn.style.pointerEvents = "auto";
  openBookPostBtn.style.pointerEvents = "auto";
  openAddImageToPostBtn.style.pointerEvents = "auto";
}
